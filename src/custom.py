...
def wallpapers(Pm: PackageManagement):
    ...
    Pm.command("wget -P ~/Images/Wallpaper https://wallpapercave.com/wp/wp12301210.jpg https://wallpapercave.com/wp/wp12099472.jpg https://wallpapercave.com/wp/wp12301247.jpg", "Downloading wallpaper Gravity Falls")

def config(Pm: PackageManagement):
    # i3 
    Pm.copy_file("config/i3/config", "~/.config/i3/config", log_msg="Copying i3 config")

    # i3status
    Pm.copy_file("config/i3/i3status.conf", "/etc/i3status.conf", sudo=True, log_msg="Copying i3status.conf")

    # Kitty
    Pm.copy_file("config/kitty/kitty.conf", "~/.config/kitty/kitty.conf", log_msg="Copying kitty.conf")
